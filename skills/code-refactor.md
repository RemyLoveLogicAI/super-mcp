...
tool_dependencies:
  (none)
composable_with: ["code-review", "code-generation"]
...
Restructure existing code to improve readability, performance, and maintainability without changing behavior.

## Description

Code Refactoring is a canonical Super-MCP skill that provides structured, repeatable capability for restructure existing code to improve readability, performance, and maintainability without changing behavior. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-refactor --request "Describe what you need"
```

### With Context

```
invoke code-refactor --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
