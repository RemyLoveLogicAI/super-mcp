...
Replay events from checkpoints for debugging and audit.

## Description

Event Replay is a canonical Super-MCP skill that provides structured, repeatable capability for replay events from checkpoints for debugging and audit. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke event-replay --request "Describe what you need"
```

### With Context

```
invoke event-replay --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
