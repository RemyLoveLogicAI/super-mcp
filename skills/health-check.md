...
Run health checks across all registered tools and services.

## Description

Health Check is a canonical Super-MCP skill that provides structured, repeatable capability for run health checks across all registered tools and services. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke health-check --request "Describe what you need"
```

### With Context

```
invoke health-check --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
