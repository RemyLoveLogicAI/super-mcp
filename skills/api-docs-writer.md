...
Generate API documentation from code, OpenAPI specs, or usage examples.

## Description

API Docs Writer is a canonical Super-MCP skill that provides structured, repeatable capability for generate api documentation from code, openapi specs, or usage examples. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke api-docs-writer --request "Describe what you need"
```

### With Context

```
invoke api-docs-writer --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
