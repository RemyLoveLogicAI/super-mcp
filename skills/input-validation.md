...
tags: ["validation", "sanitization", "xss"]
---
...
Generate input validation and sanitization logic for web applications.

## Description

Input Validation is a canonical Super-MCP skill that provides structured, repeatable capability for generate input validation and sanitization logic for web applications. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke input-validation --request "Describe what you need"
```

### With Context

```
invoke input-validation --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
