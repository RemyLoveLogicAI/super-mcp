...
Generate Nginx configurations for proxying, caching, and load balancing.

## Description

Nginx Configuration is a canonical Super-MCP skill that provides structured, repeatable capability for generate nginx configurations for proxying, caching, and load balancing. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke nginx-config --request "Describe what you need"
```

### With Context

```
invoke nginx-config --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
