...
Facilitate sprint retrospectives with structured formats (Start/Stop/Continue, 4Ls).

## Description

Retrospective Facilitator is a canonical Super-MCP skill that provides structured, repeatable capability for facilitate sprint retrospectives with structured formats (start/stop/continue, 4ls). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke retrospective --request "Describe what you need"
```

### With Context

```
invoke retrospective --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
