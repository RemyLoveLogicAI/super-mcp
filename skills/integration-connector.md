...
Build integrations between services and APIs.

## Description

Integration Connector is a canonical Super-MCP skill that provides structured, repeatable capability for build integrations between services and apis. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke integration-connector --request "Describe what you need"
```

### With Context

```
invoke integration-connector --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
