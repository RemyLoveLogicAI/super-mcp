...
Calculate code complexity, coverage, duplication, and quality metrics.

## Description

Code Metrics is a canonical Super-MCP skill that provides structured, repeatable capability for calculate code complexity, coverage, duplication, and quality metrics. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-metrics --request "Describe what you need"
```

### With Context

```
invoke code-metrics --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
