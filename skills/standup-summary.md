...
Generate daily standup summaries from git activity and task updates.

## Description

Standup Summary is a canonical Super-MCP skill that provides structured, repeatable capability for generate daily standup summaries from git activity and task updates. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke standup-summary --request "Describe what you need"
```

### With Context

```
invoke standup-summary --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
