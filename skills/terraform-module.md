...
Write Terraform modules for cloud infrastructure provisioning.

## Description

Terraform Module is a canonical Super-MCP skill that provides structured, repeatable capability for write terraform modules for cloud infrastructure provisioning. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke terraform-module --request "Describe what you need"
```

### With Context

```
invoke terraform-module --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
