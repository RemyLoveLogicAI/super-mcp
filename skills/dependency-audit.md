...
Audit project dependencies for vulnerabilities, licenses, and update paths.

## Description

Dependency Audit is a canonical Super-MCP skill that provides structured, repeatable capability for audit project dependencies for vulnerabilities, licenses, and update paths. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke dependency-audit --request "Describe what you need"
```

### With Context

```
invoke dependency-audit --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
