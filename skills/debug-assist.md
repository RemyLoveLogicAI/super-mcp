...
Systematically identify and fix bugs using stack traces, logs, and code analysis.

## Description

Debug Assistant is a canonical Super-MCP skill that provides structured, repeatable capability for systematically identify and fix bugs using stack traces, logs, and code analysis. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke debug-assist --request "Describe what you need"
```

### With Context

```
invoke debug-assist --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
