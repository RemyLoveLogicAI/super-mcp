...
Generate web scraping scripts with proper rate limiting and error handling.

## Description

Web Scraper Builder is a canonical Super-MCP skill that provides structured, repeatable capability for generate web scraping scripts with proper rate limiting and error handling. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke web-scraper --request "Describe what you need"
```

### With Context

```
invoke web-scraper --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
