...
Build, explain, and validate regular expressions with test cases.

## Description

Regex Builder is a canonical Super-MCP skill that provides structured, repeatable capability for build, explain, and validate regular expressions with test cases. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke regex-builder --request "Describe what you need"
```

### With Context

```
invoke regex-builder --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
