...
Code Translation is a canonical Super-MCP skill that provides structured, repeatable capability for translate code between programming languages while preserving logic and idioms. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-translate --request "Describe what you need"
```

### With Context

```
invoke code-translate --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
