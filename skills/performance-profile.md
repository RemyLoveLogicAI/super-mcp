...
Performance Profiling is a canonical Super-MCP skill that provides structured, repeatable capability for identify performance bottlenecks and suggest optimization strategies. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke performance-profile --request "Describe what you need"
```

### With Context

```
invoke performance-profile --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
