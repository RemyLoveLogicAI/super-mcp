...
Parse, analyze, and extract patterns from application logs.

## Description

Log Analysis is a canonical Super-MCP skill that provides structured, repeatable capability for parse, analyze, and extract patterns from application logs. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke log-analysis --request "Describe what you need"
```

### With Context

```
invoke log-analysis --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
