...
Design RESTful, GraphQL, or gRPC APIs with OpenAPI specifications.

## Description

API Design is a canonical Super-MCP skill that provides structured, repeatable capability for design restful, graphql, or grpc apis with openapi specifications. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke api-design --request "Describe what you need"
```

### With Context

```
invoke api-design --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
