...
Generate structured meeting notes with action items and decisions.

## Description

Meeting Notes is a canonical Super-MCP skill that provides structured, repeatable capability for generate structured meeting notes with action items and decisions. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke meeting-notes --request "Describe what you need"
```

### With Context

```
invoke meeting-notes --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
