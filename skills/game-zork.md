...
tags: ["zork", "text-adventure", "parser"]
---
...
Run Zork-style text adventure games with parser-based interaction.

## Description

Interactive Fiction is a canonical Super-MCP skill that provides structured, repeatable capability for run zork-style text adventure games with parser-based interaction. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke game-zork --request "Describe what you need"
```

### With Context

```
invoke game-zork --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
