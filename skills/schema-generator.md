...
Generate JSON Schema, Pydantic models, or TypeScript interfaces from sample data.

## Description

Schema Generator is a canonical Super-MCP skill that provides structured, repeatable capability for generate json schema, pydantic models, or typescript interfaces from sample data. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke schema-generator --request "Describe what you need"
```

### With Context

```
invoke schema-generator --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
