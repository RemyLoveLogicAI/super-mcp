...
Cron Scheduler is a canonical Super-MCP skill that provides structured, repeatable capability for build and explain cron expressions for scheduled tasks. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke cron-scheduler --request "Describe what you need"
```

### With Context

```
invoke cron-scheduler --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
