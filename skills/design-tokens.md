...
Generate and manage design tokens for consistent theming.

## Description

Design Tokens is a canonical Super-MCP skill that provides structured, repeatable capability for generate and manage design tokens for consistent theming. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke design-tokens --request "Describe what you need"
```

### With Context

```
invoke design-tokens --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
