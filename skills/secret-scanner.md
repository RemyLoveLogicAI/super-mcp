...
Scan code and configs for accidentally committed secrets and credentials.

## Description

Secret Scanner is a canonical Super-MCP skill that provides structured, repeatable capability for scan code and configs for accidentally committed secrets and credentials. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke secret-scanner --request "Describe what you need"
```

### With Context

```
invoke secret-scanner --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
