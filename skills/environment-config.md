...
Generate environment configurations, .env files, and secret management setups.

## Description

Environment Config is a canonical Super-MCP skill that provides structured, repeatable capability for generate environment configurations, .env files, and secret management setups. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke environment-config --request "Describe what you need"
```

### With Context

```
invoke environment-config --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
