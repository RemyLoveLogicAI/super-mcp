...
Generate diagrams (flowcharts, sequence, architecture) in Mermaid or PlantUML.

## Description

Diagram Generator is a canonical Super-MCP skill that provides structured, repeatable capability for generate diagrams (flowcharts, sequence, architecture) in mermaid or plantuml. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke diagram-generator --request "Describe what you need"
```

### With Context

```
invoke diagram-generator --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
