...
Design authentication and authorization systems (OAuth, JWT, RBAC).

## Description

Auth Design is a canonical Super-MCP skill that provides structured, repeatable capability for design authentication and authorization systems (oauth, jwt, rbac). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke auth-design --request "Describe what you need"
```

### With Context

```
invoke auth-design --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
