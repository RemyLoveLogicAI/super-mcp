...
Audit code and infrastructure for security vulnerabilities (OWASP, CWE).

## Description

Security Audit is a canonical Super-MCP skill that provides structured, repeatable capability for audit code and infrastructure for security vulnerabilities (owasp, cwe). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke security-audit --request "Describe what you need"
```

### With Context

```
invoke security-audit --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
