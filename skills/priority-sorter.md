...
Sort and prioritize tasks using frameworks (Eisenhower, RICE, MoSCoW).

## Description

Priority Sorter is a canonical Super-MCP skill that provides structured, repeatable capability for sort and prioritize tasks using frameworks (eisenhower, rice, moscow). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke priority-sorter --request "Describe what you need"
```

### With Context

```
invoke priority-sorter --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
