...
Generate responsive CSS layouts using Grid, Flexbox, and modern techniques.

## Description

Responsive Layout is a canonical Super-MCP skill that provides structured, repeatable capability for generate responsive css layouts using grid, flexbox, and modern techniques. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke responsive-layout --request "Describe what you need"
```

### With Context

```
invoke responsive-layout --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
