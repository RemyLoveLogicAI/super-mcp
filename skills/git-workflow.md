...
Generate commits, manage branches, resolve conflicts, and automate git operations.

## Description

Git Workflow is a canonical Super-MCP skill that provides structured, repeatable capability for generate commits, manage branches, resolve conflicts, and automate git operations. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke git-workflow --request "Describe what you need"
```

### With Context

```
invoke git-workflow --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
