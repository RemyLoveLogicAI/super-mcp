...
Config Validator is a canonical Super-MCP skill that provides structured, repeatable capability for validate configuration files against schemas and best practices. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke config-validator --request "Describe what you need"
```

### With Context

```
invoke config-validator --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
