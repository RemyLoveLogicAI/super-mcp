...
Organize, rename, and sort files based on configurable rules.

## Description

File Organizer is a canonical Super-MCP skill that provides structured, repeatable capability for organize, rename, and sort files based on configurable rules. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke file-organizer --request "Describe what you need"
```

### With Context

```
invoke file-organizer --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
