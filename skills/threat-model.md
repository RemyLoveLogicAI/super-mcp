...
Create STRIDE-based threat models for applications and systems.

## Description

Threat Modeling is a canonical Super-MCP skill that provides structured, repeatable capability for create stride-based threat models for applications and systems. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke threat-model --request "Describe what you need"
```

### With Context

```
invoke threat-model --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
