...
Run a full D&D campaign as an AI Dungeon Master with rule enforcement.

## Description

D&D Game Master is a canonical Super-MCP skill that provides structured, repeatable capability for run a full d&d campaign as an ai dungeon master with rule enforcement. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke game-dnd --request "Describe what you need"
```

### With Context

```
invoke game-dnd --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
