...
tags: ["notifications", "email", "slack"]
---
...
Design and implement notification systems (email, Slack, webhook).

## Description

Notification System is a canonical Super-MCP skill that provides structured, repeatable capability for design and implement notification systems (email, slack, webhook). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke notification-system --request "Describe what you need"
```

### With Context

```
invoke notification-system --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
