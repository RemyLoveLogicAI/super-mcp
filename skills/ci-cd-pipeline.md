...
Design and generate CI/CD pipeline configurations for major platforms.

## Description

CI/CD Pipeline is a canonical Super-MCP skill that provides structured, repeatable capability for design and generate ci/cd pipeline configurations for major platforms. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke ci-cd-pipeline --request "Describe what you need"
```

### With Context

```
invoke ci-cd-pipeline --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
