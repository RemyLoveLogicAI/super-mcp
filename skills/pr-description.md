...
Generate detailed pull request descriptions with context and review guidelines.

## Description

PR Description Writer is a canonical Super-MCP skill that provides structured, repeatable capability for generate detailed pull request descriptions with context and review guidelines. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke pr-description --request "Describe what you need"
```

### With Context

```
invoke pr-description --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
