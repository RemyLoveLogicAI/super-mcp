...
Compose multiple skills into a single workflow execution.

## Description

Skill Composer is a canonical Super-MCP skill that provides structured, repeatable capability for compose multiple skills into a single workflow execution. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke skill-composer --request "Describe what you need"
```

### With Context

```
invoke skill-composer --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
