...
Configure centralized logging (ELK, Loki, Fluentd).

## Description

Log Aggregation Setup is a canonical Super-MCP skill that provides structured, repeatable capability for configure centralized logging (elk, loki, fluentd). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke log-aggregation --request "Describe what you need"
```

### With Context

```
invoke log-aggregation --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
