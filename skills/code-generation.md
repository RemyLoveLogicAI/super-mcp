...
tool_dependencies:
  (none)
composable_with: ["code-review", "code-refactor"]
tags: ["generate", "write", "implement"]
---
...
Code Generation is a canonical Super-MCP skill that provides structured, repeatable capability for generate production-ready code from natural language specifications. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-generation --request "Describe what you need"
```

### With Context

```
invoke code-generation --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
