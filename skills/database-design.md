...
Design database schemas, migrations, and query optimization strategies.

## Description

Database Design is a canonical Super-MCP skill that provides structured, repeatable capability for design database schemas, migrations, and query optimization strategies. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke database-design --request "Describe what you need"
```

### With Context

```
invoke database-design --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
