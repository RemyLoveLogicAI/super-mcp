...
tags: ["decision", "matrix", "comparison"]
---
...
Build weighted decision matrices for comparing options.

## Description

Decision Matrix is a canonical Super-MCP skill that provides structured, repeatable capability for build weighted decision matrices for comparing options. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke decision-matrix --request "Describe what you need"
```

### With Context

```
invoke decision-matrix --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
