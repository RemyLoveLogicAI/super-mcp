...
Generate user-facing release notes from technical changelogs.

## Description

Release Notes Writer is a canonical Super-MCP skill that provides structured, repeatable capability for generate user-facing release notes from technical changelogs. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke release-notes --request "Describe what you need"
```

### With Context

```
invoke release-notes --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
