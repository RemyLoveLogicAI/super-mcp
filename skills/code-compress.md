...
Reduce code size while preserving functionality through minification and dead code elimination.

## Description

Code Compression is a canonical Super-MCP skill that provides structured, repeatable capability for reduce code size while preserving functionality through minification and dead code elimination. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-compress --request "Describe what you need"
```

### With Context

```
invoke code-compress --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
