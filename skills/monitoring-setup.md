...
Configure monitoring, alerting, and dashboards (Prometheus, Grafana, DataDog).

## Description

Monitoring Setup is a canonical Super-MCP skill that provides structured, repeatable capability for configure monitoring, alerting, and dashboards (prometheus, grafana, datadog). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke monitoring-setup --request "Describe what you need"
```

### With Context

```
invoke monitoring-setup --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
