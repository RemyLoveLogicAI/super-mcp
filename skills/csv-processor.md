...
Process, filter, aggregate, and transform CSV files.

## Description

CSV Processor is a canonical Super-MCP skill that provides structured, repeatable capability for process, filter, aggregate, and transform csv files. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke csv-processor --request "Describe what you need"
```

### With Context

```
invoke csv-processor --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
