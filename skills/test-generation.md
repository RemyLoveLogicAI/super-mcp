...
Generate comprehensive test suites (unit, integration, e2e) for existing code.

## Description

Test Generation is a canonical Super-MCP skill that provides structured, repeatable capability for generate comprehensive test suites (unit, integration, e2e) for existing code. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke test-generation --request "Describe what you need"
```

### With Context

```
invoke test-generation --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
