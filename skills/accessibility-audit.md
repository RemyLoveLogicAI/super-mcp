...
Audit web content for WCAG compliance and accessibility issues.

## Description

Accessibility Audit is a canonical Super-MCP skill that provides structured, repeatable capability for audit web content for wcag compliance and accessibility issues. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke accessibility-audit --request "Describe what you need"
```

### With Context

```
invoke accessibility-audit --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
