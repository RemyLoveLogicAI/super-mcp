...
Report Super-MCP system status including all registries, tools, and sessions.

## Description

System Status is a canonical Super-MCP skill that provides structured, repeatable capability for report super-mcp system status including all registries, tools, and sessions. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke system-status --request "Describe what you need"
```

### With Context

```
invoke system-status --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
