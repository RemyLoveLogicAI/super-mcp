...
Generate and audit HTTP security headers (CSP, HSTS, CORS).

## Description

Security Headers is a canonical Super-MCP skill that provides structured, repeatable capability for generate and audit http security headers (csp, hsts, cors). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke security-headers --request "Describe what you need"
```

### With Context

```
invoke security-headers --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
