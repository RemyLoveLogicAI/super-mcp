...
Design and generate data pipeline configurations (Airflow, dbt, Prefect).

## Description

Data Pipeline Builder is a canonical Super-MCP skill that provides structured, repeatable capability for design and generate data pipeline configurations (airflow, dbt, prefect). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke data-pipeline --request "Describe what you need"
```

### With Context

```
invoke data-pipeline --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
