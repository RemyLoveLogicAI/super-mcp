...
Search codebases for patterns, usages, definitions, and references.

## Description

Code Search is a canonical Super-MCP skill that provides structured, repeatable capability for search codebases for patterns, usages, definitions, and references. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke code-search --request "Describe what you need"
```

### With Context

```
invoke code-search --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
