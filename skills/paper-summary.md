...
Summarize academic papers, extracting key findings, methodology, and limitations.

## Description

Paper Summarizer is a canonical Super-MCP skill that provides structured, repeatable capability for summarize academic papers, extracting key findings, methodology, and limitations. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke paper-summary --request "Describe what you need"
```

### With Context

```
invoke paper-summary --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
