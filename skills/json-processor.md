...
Query, transform, and validate JSON documents using JMESPath/JSONPath.

## Description

JSON Processor is a canonical Super-MCP skill that provides structured, repeatable capability for query, transform, and validate json documents using jmespath/jsonpath. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke json-processor --request "Describe what you need"
```

### With Context

```
invoke json-processor --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
