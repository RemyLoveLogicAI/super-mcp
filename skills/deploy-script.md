...
Generate deployment scripts for various platforms and strategies (blue-green, canary).

## Description

Deployment Script is a canonical Super-MCP skill that provides structured, repeatable capability for generate deployment scripts for various platforms and strategies (blue-green, canary). It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke deploy-script --request "Describe what you need"
```

### With Context

```
invoke deploy-script --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
