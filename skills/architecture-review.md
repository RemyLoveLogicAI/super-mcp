...
Review system architecture for scalability, maintainability, and adherence to patterns.

## Description

Architecture Review is a canonical Super-MCP skill that provides structured, repeatable capability for review system architecture for scalability, maintainability, and adherence to patterns. It integrates with the kernel's safety layer, event bus, and checkpoint system to ensure all operations are auditable and replayable.
...
```
invoke architecture-review --request "Describe what you need"
```

### With Context

```
invoke architecture-review --request "Specific task" --context '{"path": "/src", "lang": "python"}'
```
